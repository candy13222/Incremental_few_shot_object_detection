...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.vit_model, _ = clip.load("ViT-L/14@336px", device=device)
        self.vit_model.eval()  # 设置为评估模式
        for param in self.vit_model.parameters():
            param.requires_grad = False  # 冻结CLIP模型参数
        # 定义线性层用于投影class token的维度
        self.class_token_proj = nn.Linear(768, d_model)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None,text_embedding=None,padding_list=None, raw_images=None):
        ...
        if self.vlm: 
            ...
            if self.nb:
                tgt_novel = torch.cat([tgt, text_embedding_novel, class_token], dim=1)
