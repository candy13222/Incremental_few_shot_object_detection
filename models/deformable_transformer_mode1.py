...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None,text_embedding=None,padding_list=None):
        ...
        if self.vlm: 
            ...
            if self.nb:
                if text_embedding_novel.shape[1] < tgt.shape[1]:
                    # 如果 text_embedding_novel 的数量小于 tgt 的数量，则需要填充
                    padding_size = tgt.shape[1] - text_embedding_novel.shape[1]
                    text_embedding_novel_padded = F.pad(text_embedding_novel, (0, 0, 0, padding_size), mode='constant', value=0)
                tgt_novel = tgt + text_embedding_novel_padded
