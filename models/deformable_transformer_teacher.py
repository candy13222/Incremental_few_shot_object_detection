...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=False, two_stage_num_proposals=300,vlm=False,nb =False):
        super().__init__()
        self.vlm = vlm
        self.nb = nb
        ...
        if self.vlm:
            ...
            with torch.no_grad():
                ...
                for v in dataset_classes_name_list.keys():
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None ,text_embedding=None,padding_list=None, raw_images=None):
        ...
        if self.two_stage:
            ...
            init_reference_out = reference_points
            # print(tgt.shape,query_embed.shape)
        ...
        if self.vlm:
            ...
            class_text_embed = self.class_text_embed.unsqueeze(0).expand(tgt.shape[0],-1,-1)
            ##
            # text_embedding_strengthening,_ = self.feature_attention(text_embedding,class_text_embed,class_text_embed)
            # text_embedding = text_embedding + text_embedding_strengthening
            # text_embedding = self.feature_norm(text_embedding)
            text_embedding_strengthening,_ = self.feature_attention(text_embedding,class_text_embed,class_text_embed)
            ...
            # 假設 raw_images 是 Tensor，形狀為 (batch_size, 3, H, W)
            if isinstance(raw_images, torch.Tensor):
                raw_images = F.interpolate(raw_images, size=(336, 336), mode='bilinear', align_corners=False)  # 調整大小
                raw_images = raw_images / 255.0  # 將像素值歸一化到 [0, 1]
                raw_images = transforms.Normalize(
                    mean=[0.48145466, 0.4578275, 0.40821073],
                    std=[0.26862954, 0.26130258, 0.27577711]
                )(raw_images)

            image_features = self.vit_model.encode_image(raw_images)  # src 应该是图像张量 [batch_size, 3, H, W]
            class_token = self.class_token_proj(image_features.float())  # 投影到 d_model [batch_size, 256]
            class_token = class_token.unsqueeze(1)  # 增加时间步维度 [batch_size, 1, 256]
        
            tgt = torch.cat([tgt, text_embedding_base, class_token], dim=1)
